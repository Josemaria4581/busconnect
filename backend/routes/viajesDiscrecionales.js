...
router.get("/:id/chat", async (req, res, next) => {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM mensajes_chat WHERE viaje_id = ? ORDER BY fecha_envio ASC",
      [req.params.id]
    );
    res.json(rows);
  } catch (error) {
...
router.post("/:id/chat", async (req, res, next) => {
  try {
    ...
    if (!emisor || !mensaje) return res.status(400).json({ error: "Faltan campos" });
    const [result] = await pool.query(
      "INSERT INTO mensajes_chat (viaje_id, emisor, mensaje) VALUES (?, ?, ?)",
      [req.params.id, emisor, mensaje]
    );
    const [rows] = await pool.query("SELECT * FROM mensajes_chat WHERE id = ?", [result.insertId]);
    res.status(201).json(rows[0]);
  } catch (error) {
    next(error);
  }
